	...
	public function finalize() {
		...
		switch($this->exportTo) {
			...
			case 'file':
				...
				chmod($this->filename,0644);//赋予文件权限防止生成后下载不了 add by guanyongjun 2013-06-13
				break;
		}
	}
...
class ExportDataExcel extends ExportData {
	...
	private function generateCell($item) {
		...
		//字符串编码的问题,add by zyp 2014/10/5
		$mb_type	=	htmlspecialchars($item, ENT_QUOTES);
		if(empty($mb_type) && !empty($item)) {
			$encode	=	mb_detect_encoding($item,array('UTF-8','GBK'));
			if($encode != 'UTF-8') {
