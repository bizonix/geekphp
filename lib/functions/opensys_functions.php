 ...
 */
function vita_get_url_content2($url, $vars=''){
	$ch	=	curl_init();
	curl_setopt($ch,CURLOPT_URL,$url);
...
function createSign2($paramArr, $app_secret='') {
    if($app_secret == ''){
		$app_secret  = C('OPEN_SYS_TOKEN'); //用户purchase token
	}
    $str = ""; 
    ksort($paramArr); 
    foreach ($paramArr as $key => $val) { 
...
function callOpenSystem($paramArr){	
	...
	//连接超时自动重试3次
	$cnt=0;
    if($paramArr['getOrPost'] == '2'){ //POST方式
        while($cnt < 3 && ($result=@vita_get_url_content2($url.'sign='.createSign2($paramArr,$token), $paramArr))===FALSE) $cnt++;
    }else{ //GET方式
