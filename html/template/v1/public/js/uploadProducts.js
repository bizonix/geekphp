$(function(){
  //上传主图片
  var swfu1 = uploadPic("mainImg","mainBtnCancel1","mainImgProgress",getSkuName('-G','sku'),'',uploadMainSuccess);
  //上传属性图片
  var swfu2 = uploadPic("propImg","propBtnCancel1","propImgProgress",getSkuName('-P','sku'),'rand',uploadPropSuccess);
  deleteImg($("img[role-remove]"));
});
//上传图片核心方法
...
function uploadMainSuccess(file, serverData) {//后台文件的回调函数
	try {
		...
		if(data.errCode == "200"){
			...
			var str = '<div role-mainImg style="margin:5px;display:inline-block;"><img role-remove style="position: absolute;" src="/images/remove.png"/><a target="_blank" href="http://image.huanhuan365.com/tmpImg/'+getCompanyId()+'/'+data.data+'"><img style="max-width:200px;max-height:100px;" src="http://image.huanhuan365.com/tmpImg/'+getCompanyId()+'/'+data.data+'" /></a><p role-imgName>'+data.data+'</p><input type="hidden" name="mainImgName[]" value="'+data.data+'" /></div>';
			$("div[role-mainImgDiv]").append(str);
			deleteImg($("div[role-mainImgDiv]").find("div[role-mainImg]:last").find("img[role-remove]"));
		}else{
...
function uploadPropSuccess(file, serverData) {//后台文件的回调函数
	try {
		...
		if(data.errCode == "200"){
			progress.setStatus("上传成功");
			var str = '<div role-propImg style="margin:5px;display:inline-block;"><img role-remove style="position: absolute;" src="/images/remove.png"/><a target="_blank" href="http://image.huanhuan365.com/tmpImg/'+getCompanyId()+'/'+data.data+'"><img style="max-width:200px;max-height:100px;" src="http://image.huanhuan365.com/tmpImg/'+getCompanyId()+'/'+data.data+'" /></a><p role-imgName>'+data.data+'</p><input type="hidden" name="propImgName[]" value="'+data.data+'" /></div>';
			$("div[role-propImgDiv]").append(str);
			deleteImg($("div[role-propImgDiv]").find("div[role-propImg]:last").find("img[role-remove]"));
		}else{
			progress.setStatus("图片上传失败！");
		}
		progress.toggleCancel(false);
...
function getCompanyId(){
	return $.trim($("input[name='companyId']").val());
}
...
function deleteImg(obj){
	obj.on("click",function(){
		var companyId   = getCompanyId();
		var imgName 	= $.trim($(this).siblings('p[role-imgName]').text()).split(".");
		obj.parent().remove();
		if($.trim(companyId) && $.trim(imgName)){
			$.ajax({
		        type  : "GET",
		        async : false,
		        url   : "/images/delTmpPic/companyId/"+companyId+"/imgName/"+imgName[0],
		        dataType : "json",
		        crossDomain: true,
