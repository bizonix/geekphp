...
class ShopsAct extends CheckAct {
	 ...
	 * 和 平台(modify wcx)
	 */
	public function act_getCompanyAllShops($companyId,$platform=''){
		$where 	  = array("belong_company" => $companyId);
		if(!empty($platform)){
	 ...
	 * 获取所有店鋪信息 根据公司ID
	 */
	public function act_getAllShopsBycompanyId($companyId,$platform=''){
		$where 	  = array("belong_company" => $companyId);
		if(!empty($platform)){
			$where['platform'] = $platform;
		}
		$shopInfo = M("Shops")->getAllData("id,shop_account",$where,"id");
		if(empty($shopInfo)){
			self::$errMsg['10007']	= get_promptmsg('10007','店铺');
			return false;
		}
		foreach ($shopInfo as $key => $value) {
			$shopInfo[$key] = $value['shop_account'];
		}
		return $shopInfo;
	}
