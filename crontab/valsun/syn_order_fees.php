...
for ($i=1;$i<=$page;$i++){
	$updateOrders = array();
	$orders = M("Order")->key("id")->getData("id,simple_detail,order_id,source_platform,handle_status",$whereData,' order by id asc ',$i,$perPage);
	...
	foreach($retData as $k=>$v){
		...
		$updateData = array('id' => $orders[$k]['id'],"simple_detail"=>json_encode($simpleDetail));
	...
	if(!empty($updateOrders)){
		$updateRes = A('Order')->synOrderSomeInfo($updateOrders);
		log::writeLog("syn_order_fees = ".json_encode($updateRes),"orders/valsun/3","system_syn_order",'d');
	}
}
