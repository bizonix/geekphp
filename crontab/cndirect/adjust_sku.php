...
$ss = "SELECT * FROM om_adjust_sku WHERE CN_status=0 and type=1 and is_exception=0";
$ss = $dbConn->query($ss);
...
if(!empty($sku_stock)){
	...
	if($info['ACK'] == "Success"){
		print_r($sku_stock);
		foreach($ss as $val){
			//if($val['type']==1&&$val['CN_status']==0){
				$sql = "UPDATE om_adjust_sku SET CN_status=1 WHERE id='{$val['id']}'";
