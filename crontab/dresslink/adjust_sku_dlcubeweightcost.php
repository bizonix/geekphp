...
echo "调用DL 新接口,时间".date("Y-m-d H:i:s")."\r\n";
$ss = "SELECT * FROM om_adjust_sku WHERE DL_status=0 and type=5 and is_exception=0 order by createdtime ASC";
$ss = $dbConn->query($ss);
...
foreach($ss as $key=>$value){
	...
	if($value['type']==5){
		...
		if(count($datainfo) ==3 && count($data) ==4){
...
//新接口
//echo count($sku_cubeweightcost);
//exit;
//print_r($sku_cubeweightcost);
if(!empty($sku_cubeweightcost)){
	...
	if(isset($msg['ACK'])&&$msg['ACK'] != "Failure"){
		foreach($sku_cubeweightcost as $key=>$value){
			$sql = "UPDATE om_adjust_sku SET DL_status=1 WHERE type=5 and sku='{$key}' and is_exception=0 and DL_status=0";
			//echo $sql;
			$sql = $dbConn->query($sql);
			//$data = explode(",",$value); 
			if($sql){
				
				echo "料号【{$key}】post to dresslink 成功！ 体积：{$value['cubage']},体积重量：{$value['cubage_weight']},称重重量：{$value['weight']},货本：{$value['cost']}\r\n";
			}else{
				echo "料号【{$key}】post to dresslink 失败！ 体积：{$value['cubage']},体积重量：{$value['cubage_weight']},称重重量：{$value['weight']},货本：{$value['cost']}\r\n";
			}
		}
	}else{
