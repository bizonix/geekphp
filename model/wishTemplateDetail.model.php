<?php
/**
 * 类名：WishTemplateDetailModel
 * 功能：wish范本详情表管理
 * 版本：V1.0
 * 作者：邹军荣
 * 时间：2015-06-11
 */
class WishTemplateDetailModel extends CommonModel{
	...
	public function buildSaveData($params){
	    $retData = array();
	    if(!empty($params)){
	        $tp_id = isset($params['tp_id']) && !empty($params['tp_id']) ? trim($params['tp_id']) : '';
	        if(!empty($tp_id)){
	            $retData['tp_id'] = $tp_id;
	        }
	...
	public function buildWhereData($params){
	    ...
	    if(!empty($params)){
	        ...
	        if(isset($params['tp_id'])) $retData['tp_id'] = $params['tp_id'];
	 ...
	 * zjr
	 */
	public function saveTemplateDetailData($data,$subfix){
	    $ret = 0;
	    //判断数据是否存在
	    $where = array(
	        'tp_id'        => $data['tp_id'],
